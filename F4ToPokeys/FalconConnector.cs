    ...
    public class FalconConnector
    {
        ...
        private void initLightList()
        {
            ...
            addToLightList(new FalconBlinkingLamp(threatWarningAuxiliary, "SEARCH",
                flightData => (flightData.lightBits2 & (int)LightBits2.AuxSrch) != 0 & (int)LightBits2.AuxPwr != 0, BlinkBits.AuxSrch, 250));
        ...
        private void initGaugeList()
        {
            ...
            addToGaugeList(new FalconGauge("ALT100", flightData => (-flightData.aauz % 1000) - (-flightData.aauz % 100) + (-flightData.aauz % 100) - (-flightData.aauz % 10) + (-flightData.aauz % 10) - (-flightData.aauz % 1), 0.0F, 999.9F, 3, 0, 0));
            addToGaugeList(new FalconGauge("ALT1000", flightData => (-flightData.aauz % (100000)) - (-flightData.aauz % 10000) + (-flightData.aauz % (10000)) - (-flightData.aauz % 1000), 0.0F, 99999.9F, 5, 0, 0));
